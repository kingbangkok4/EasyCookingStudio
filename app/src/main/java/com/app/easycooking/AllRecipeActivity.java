...
/*	private String specific_add_id = "";
	...
	private void DialogEditRecipe(final String recipe_id, final String recipe_name, String recipe_specific, String recipe_calories, String recipe_how_to_cook, String specific_id) {
		...
		ckLessSugar = (CheckBox) checkBoxView.findViewById(R.id.ckbLessSugar);
		ckLessCarbohydrate = (CheckBox) checkBoxView.findViewById(R.id.ckbLessCarbohydrate);
		ckLessSalt = (CheckBox) checkBoxView.findViewById(R.id.ckbLessSalt);
...
/*		specificList = myDB.GetSpecific();
		...
		if(!"".equals(recipe_calories)) {
			recipe_calories = recipe_calories.replace("Calories", "");
		...
		AlertDialog.Builder builderInOut = new AlertDialog.Builder(this);
		builderInOut.setTitle("Edit Recipe");
		builderInOut.setMessage("")
				.setView(checkBoxView)
				.setCancelable(false)
				.setPositiveButton("Close", new DialogInterface.OnClickListener() {
...
/*		final Spinner spSpecific = (Spinner)checkBoxView.findViewById(R.id.spinnerSpecific);*/
		...
		btAddNewRecipe.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!"".equals(strName) && !"".equals(strCalories) && arraySpecific.length > 0) {
					...
					if("Add recipe completed".equals(statusMsg)){
						eRecipeName.setText("");
						eCalories.setText("");
					}
				}else{
		...
		AlertDialog.Builder builderInOut = new AlertDialog.Builder(this);
		builderInOut.setTitle("Add Recipe");
		builderInOut.setMessage("Please input information")
				.setView(checkBoxView)
				.setCancelable(false)
				.setPositiveButton("Close", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
	...
	private void SetSpecificFormID(String specific_id) {
		...
		for (int i = 0; i < arraySpecificId.length; i++) {
			...
			}else if("3".equals(arraySpecificId[i].toString())){
				ckLessSugar.setChecked(true);
			}else if("4".equals(arraySpecificId[i].toString())){
				ckLessCarbohydrate.setChecked(true);
			}else if("5".equals(arraySpecificId[i].toString())){
				ckLessSalt.setChecked(true);
			}
		}
	}
